...
export class RestService {
    ...
    getMessages() {
        const {_page, _limit} = this.route.snapshot.queryParams;
        return this.http.get<Message[]>(`/messages?_page=${+_page || 1}&_limit=${+_limit || 10}`, { observe: 'response' }).pipe(
    ...
    getSearchResults() {
        this.messagesLoading$.next(true);
        const { _limit, q } = this.route.snapshot.queryParams;
        return this.http.get<Message[]>(`/messages?q=${q}&_page=1&_limit=${+_limit || 20}`, { observe: 'response' }).pipe(
